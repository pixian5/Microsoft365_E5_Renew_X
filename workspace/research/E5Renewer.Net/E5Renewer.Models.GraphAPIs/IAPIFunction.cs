    ...
    internal async Task<APICallResult> SafeCallAsync(GraphServiceClient client, string user)
    {
        static string BuildFailureToken(string code, string? message)
        {
            ...
            string? reason = normalizedMessage switch
            {
                ...
                _ when normalizedMessage.Contains("Raw count (/$count) is not supported because it is used with the unsupported entity", StringComparison.OrdinalIgnoreCase) => "UnsupportedEntityCount",
                ...
                _ when normalizedMessage.Contains("does not have access to this report or data", StringComparison.OrdinalIgnoreCase) => "MissingLicense",
                _ when normalizedMessage.Contains("share id was malformed", StringComparison.OrdinalIgnoreCase) => "MalformedResourceIdentifier",
                _ when normalizedMessage.Contains("Request url was invalid", StringComparison.OrdinalIgnoreCase) => "InvalidRequestUrl",
                _ when normalizedMessage.Contains("No HTTP resource was found that matches the request URI", StringComparison.OrdinalIgnoreCase) => "MissingResourcePath",
                _ when normalizedMessage.Contains("This API is not supported for AAD accounts", StringComparison.OrdinalIgnoreCase) => "PartnerTenantRequired",
                ...
                _ when normalizedMessage.Contains("not onboarded", StringComparison.OrdinalIgnoreCase) => "TenantNotOnboarded",
